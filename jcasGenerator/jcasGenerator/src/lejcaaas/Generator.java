...
//	private static boolean typeProg = false;
		...
		if( typeProg == true) {
			if( type == null) {
				...
				if( (i = natureIdent.indexOf("array")) != -1) { // pr�sence d'un array pour le doublearray
					...
					if( affec.equals(" := ")) {
						text = tab.concat(variable).concat("[1]").concat(affec).concat(varUsed.get(i)).concat(eol).concat("\n"); 
					}
					else if(( varUsed.get(i).equals("Integer") || varUsed.get(i).contains("..")) && (affec.equals(" div ") || affec.equals(" mod "))) {
			...
			else if( type == "integer" || type == "real" || type.contains("..")) {
				String affec = characGenerator("affect");
				if( affec.contains(" := ") && (type == "integer" || type.contains(".."))) {
					text = tab.concat(variable).concat(affec).concat(String.valueOf((int)(Math.random()*max_int))).concat(eol).concat("\n");
				}
				else if( affec.contains(":=") && type == "real"){
					text = tab.concat(variable).concat(affec).concat(String.valueOf((Math.random()*max_int))).concat(eol).concat("\n");
				
				}
				else if( !affec.equals(" div ") && !affec.equals(" mod ")) {
					text = tab.concat(variable).concat(" := " ).concat(String.valueOf((Math.random()*max_int))).concat(affec).concat(variable).concat(eol).concat("\n");
				}
				else {
					//On fiat simple affection dans le cas de div/ mod
					if( type == "integer" || type.contains("..")) {
						text = tab.concat(variable).concat(" := ").concat(variable).concat(affec).concat(variable).concat(eol).concat("\n");
					}
					else {

						text = tab.concat(variable).concat(" := ").concat(String.valueOf((Math.random()*max_int))).concat(eol).concat("\n");
					}
				}
			}
	...
	public String motSemantique(String mot) {
		...
		if( mot == "write") {
			...
			if( i == -1) {
				...
				for(int j=0; j<typeVar.size(); j++) {
					if ( (typeVar.get(j).equals("integer") || typeVar.get(j).contains("..") || typeVar.get(j).equals("real")) && i<=2 && typeVar.size() > 1) {
						...
						text = text.concat(compareGenerator()).concat(String.valueOf((int)(Math.random()*max_int))).concat(")").concat(tab).concat("do").concat("\n");
						break;
					}
			...
			if( var2 == null && var1 != null) {
				...
				if( var2 != null && var1 != null) {
					//System.out.println("VAR2 " + natureIdent.get(varUsed.indexOf(var2)));
					//System.out.println("var2 " + var2 + " nature " +natureIdent.get(varUsed.indexOf(var2)));
					text = text.concat(tab).concat(bodyVarBeautify(var2,natureIdent.get(varUsed.indexOf(var2))));
				}
			...
			if( i == -1) {
				...
				i = 1;
				for(int j=0; j<typeVar.size(); j++) {
					if ( (typeVar.get(j).equals("integer") || typeVar.get(j).equals("real") ) && i<2) {
				...
				var1 = varUsed.get(i);
				text = text.concat(var1).concat(")").concat(tab).concat("then").concat("\n").concat(tab);
			}
			//Génération du body
			if( var2 == null && var1 != null) {
				// boolean
				text = text.concat(bodyVarBeautify(var1,typeVar.get(varUsed.indexOf(var1))));
			}
			else if( var2 != null) {
				//System.out.println("var 2 " + var2 + " " + typeVar.get(varUsed.indexOf(var2)));
				text = text.concat(bodyVarBeautify(var2,typeVar.get(varUsed.indexOf(var2))));

			}
			else {
				text = text.concat(bodyVarBeautify(varUsed.get(0),typeVar.get(varUsed.indexOf(varUsed.get(0)))));

			}
			text = text.concat(tab).concat("end ;").concat("\n");
			
		}
		else if( mot == "for" &&  varUsed.size() != 0) {
			i = typeVar.indexOf("integer");
			if( i == -1) {
				//Pas de for possible on termine
				text = commentGenerator();
				text = text.concat(tab).concat("null ;").concat("\n");

			}
			else {
				text = tab.concat(mot).concat(" " + varUsed.get(i)).concat(" := 1 to 20 do").concat("\n");
				text = text.concat(tab).concat(bodyVarBeautify(varUsed.get(i),typeVar.get(varUsed.indexOf(varUsed.get(i)))));
				text = text.concat(tab).concat("end ;").concat("\n");
			}
